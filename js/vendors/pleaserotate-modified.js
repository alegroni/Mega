(function () {
    ...
    var options = {
        ...
        onShow: function(){},
        forcePortrait: false,
        message: "Por favor, girá tu dispositivo",
        subMessage: "",
    ...
    function isPortrait() {
        if (window.frameElement)
            return parent.window.innerWidth < parent.window.innerHeight;
        else
    ...
    PleaseRotate.start = function(opts){
        ...
        if (window.frameElement) {
            window.addEventListener('orientationchange', delayCheck, false);
        } else {
            window.addEventListener('resize', delayCheck, false);
        }

        function delayCheck() {
            setTimeout(checkOrientationChange, 500);
        }
